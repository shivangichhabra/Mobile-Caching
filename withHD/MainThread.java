...
//						}
			...
			for (String node : mru.cache.keySet()) 
			{
			...
			for (String node : mru.hardDisk.keySet()) 
			{
				int temperature = mru.getDataAtCoordinatesHD(node).tempValue;
				int frequency = mru.getDataAtCoordinatesHD(node).frequency;
				System.out.println("(x, y, temperature,frequency) : " + node + " " + temperature + " " + frequency);
