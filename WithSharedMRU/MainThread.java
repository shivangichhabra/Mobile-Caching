...
public class MainThread
{
	...
	{	
		...
		MRUCaching mru = new MRUCaching(CAPACITY/2);
		MRUCaching sharedmru = new MRUCaching(CAPACITY/2);

		myport = sr.nextInt();

		// send's connection request
		Server server = new Server(myport, mru,sharedmru);
		...
		LookupRequestThread lookuprequest = new LookupRequestThread(mru,sharedmru,neighborPort);
				...
				try 
				{
					...
					else
					{
						...
						else
						{
							...
							if(sharedmru.containsCoordinates(location))
							{
								Data oldValues = sharedmru.getDataAtCoordinates(location);
						...
						//{
							sharedmru.insert(received.getKey(), received);
						//}
