...
public class Lookup 
{
	...
	private MRUCaching sharedmru;
	ArrayList<Integer>  neighborPort;

	public Lookup( MRUCaching mru, MRUCaching sharedmru,ArrayList<Integer>  neighborPort)
	{
		this.mru = mru;
	...
	public void LookupWithCoordinates(String input)
	{
		...
		{
			Node node = sharedmru.get(input);
			System.out.println("\nMATCH FOUND LOCALLY:" + node); //Shared with robot
			hit++;
			System.out.println("Cache Hit Rate: " + cacheHitRate() + "%\n");
		}
		else
		{
			...
			for(i = 0; i < neighborPort.size(); i++)
			{
				...
				try 
				{
					...
					{
						sharedmru.insert(input, received);
